...
const slice = createSlice({
  ...
  reducers: {
    ...
    setExecutingNode(s, a) {
      s.executingNode = a.payload;
    },
    handleError(s, a) {
      s.error = a.payload;
      s.messages.push({
        role: "assistant",
        content: `Error: ${a.payload}`,
        action: "error",
      });
    },
    handleResult(s, a) {
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      if (file) {
        ...
        fd.append(
          "message",
          message || "Extract complaint details from this document."
        );
        ...
        await streamIntake(
          "/ai/chat",
          { json: { message, history, form_state: chat.form } },
          onEvent
        );
      }
    } catch (err) {
